...
async function initialize() {
    ...
    const stripe_invoice_id = document.getElementById("stripe_invoice_id").value;
    const stripe_customer = document.getElementById("stripe_customer").value;
    const { id, clientSecret } = await fetch("payment_init.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ request_type:'create_payment_intent', "price" : price, "stripe_invoice_id" : stripe_invoice_id, "stripe_customer" : stripe_customer, }),
    }).then((r) => r.json());
...
async function handleSubmit(e) {
    ...
    const { id, customer_id } = await fetch("payment_init.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ request_type:'create_customer', payment_intent_id: payment_intent_id, email: customer_email }),
    }).then((r) => r.json());
        ...
        switch (paymentIntent.status) { 
            case "succeeded":
                ...
                fetch("payment_init.php", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ request_type:'payment_insert', payment_intent: paymentIntent, customer_id: customerID, invoice_id : invoice_id , stripe_invoice_id : stripe_invoice_id, stripe_customer : stripe_customer, }),
                })
                                ...
                                Swal.fire({
                                  ...
                                  text: "Invoice Paid Successfully!",
                                    ...
                                    if(role == "admin"){
                                        ...
                                        window.location.href = '../vet_invoices.php';
                                    }
                                });
